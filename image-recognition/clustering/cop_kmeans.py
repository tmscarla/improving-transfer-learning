...
def cop_kmeans(dataset, k, ml=[], cl=[],
    ...
    ml_graph = defaultdict(set)
    cl_graph = defaultdict(set)
    for edge in ml:
        ml_graph[edge[0]].add(edge[1])
    for edge in cl:
        cl_graph[edge[0]].add(edge[1])
    ml, cl = ml_graph, cl_graph
    ...
    for _ in tqdm(range(max_iter), desc='COP-KMEANS', disable=not verbose):
