...
if __name__ == '__main__':
    ...
    source_model_name = 's_0={}-i_0={:.1f}-r_0={}_beta={:.1f}_gamma={:.1f}_noise_0.pt'.format(
        source_s_0_bundle, source_i_0,
        source_r_0, source_beta, source_gamma)

    try:
        # It tries to load the model, otherwise it trains it
        checkpoint = torch.load(
            ROOT_DIR + '/models/SIR_bundle_init/{}'.format(source_model_name))
    ...
    target_model_name = 's_0={}-i_0={:.1f}-r_0={}_beta={:.1f}_gamma={:.1f}_noise_0.pt'.format(
        target_s_0_bundle, target_i_0,
        target_r_0, target_beta, target_gamma)

    try:
        checkpoint = torch.load(
            ROOT_DIR + '/models/SIR_bundle_init/{}'.format(target_model_name))
